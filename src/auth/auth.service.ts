...
export class AuthService {
  private pendingVerificationEmails: Map<number, string> = new Map();
  private pendingPasswordResetEmails: Map<number, string> = new Map();
  ...
  async createUserLoggedInByGoogle(
  ...
  async signup(
    ...
    try {
      ...
      // Stocker l'email pour la vérification avec l'ID de l'utilisateur
      this.pendingVerificationEmails.set(savedUser.id, email);
  ...
  async verifyEmailWithOtp(otp: string): Promise<boolean> {
    // Rechercher l'utilisateur qui a ce token OTP valide pour la vérification d'email
    const users = await this.userRepository.find();

    for (const user of users) {
      if (this.pendingVerificationEmails.has(user.id)) {
        ...
        if (isValid) {
          ...
          this.pendingVerificationEmails.delete(user.id);
          return true;
        }
      }
    }
  ...
  async generatePasswordResetOTP(email: string) {
    ...
    try {
      ...
      // Stocker l'email avec l'ID utilisateur pour la réinitialisation
      this.pendingPasswordResetEmails.set(user.id, email);
  ...
  async verifyPasswordResetOTP(otp: string): Promise<boolean> {
    // Rechercher l'utilisateur qui a une réinitialisation en attente
    const users = await this.userRepository.find();

    for (const user of users) {
      if (this.pendingPasswordResetEmails.has(user.id)) {
        const isValid = await this.verificationTokenService.validateOtp(
          user.id,
          otp,
        );
  ...
  async resetPassword(newPassword: string): Promise<boolean> {
    // Trouver l'utilisateur qui a une réinitialisation en attente
    const users = await this.userRepository.find();
    ...
    for (const user of users) {
      if (this.pendingPasswordResetEmails.has(user.id)) {
        try {
          ...
          // Supprimer l'email en attente
          this.pendingPasswordResetEmails.delete(user.id);
          return true;
        } catch (error) {
          throw new InternalServerErrorException(
    ...
    throw new UnprocessableEntityException(
      'Aucune réinitialisation en attente',
    );
  }
